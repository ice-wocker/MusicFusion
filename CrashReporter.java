...
public class CrashReporter {
    ...
    private static JSONObject buildCrashJson(Context ctx, Thread thread, Throwable e) {
        ...
        try {
            ...
            // 隐私保护: 堆栈脱敏 - 移除可能的文件路径和包名前缀
            String stack = sw.toString();
            stack = stack.replaceAll("(/data/(data|user|app)/[^\\s]+)", "<sandbox>");
            stack = stack.replaceAll("(/storage/emulated/0/[^\\s]+)", "<sdcard>");
            stack = stack.replaceAll("(/system/[^\\s]+)", "<system>");
            o.put("stack_trace", stack);
            // 设备信息 - 全部脱敏 (SHA256 截断)
            JSONObject dev = new JSONObject();
            dev.put("model_hash", hash16(Build.MODEL + Build.BRAND + Build.DEVICE));
            dev.put("android_version", Build.VERSION.RELEASE);
            dev.put("sdk_int", Build.VERSION.SDK_INT);
            dev.put("fingerprint_hash", hash16(Build.FINGERPRINT));
            dev.put("abi", Build.SUPPORTED_ABIS != null && Build.SUPPORTED_ABIS.length > 0 ? Build.SUPPORTED_ABIS[0] : "?");
            o.put("device", dev);
            // 应用信息
